...
def plot_chart(selected_ticker, selected_metric):
    ...
    fig.add_trace(
        go.Bar(x=data_filtered_by_ticker['Date'], y=data_filtered_by_ticker[selected_metric], name=selected_metric,
               width=20 * 86400000, marker_color='#0F1B93'))
    fig.add_trace(go.Scatter(x=data_filtered_by_ticker['Date'], y=data_filtered_by_ticker[metric_yoy], name=metric_yoy,
                             line=dict(dash='dot', color='#E67E22', width=4), yaxis='y2'))

    # Update layout to include a secondary y-axis for percentages and other adjustments
    fig.update_layout(
        yaxis=dict(title=f'<b>{selected_metric}</b>'),
        yaxis2=dict(title=f'<b>{metric_yoy}</b>', overlaying='y', side='right', tickformat='.1%', showgrid=False,
                    zeroline=False),
...
st.plotly_chart(plot_chart(selected_ticker1, selected_metric))

# Plotting the second chart
st.plotly_chart(plot_chart(selected_ticker2, selected_metric))